...
/* INGRESAR DATOS (NOMBRE, MAIL, TARJETA, MONTO) */

const agregarMonto = document.createElement("button")
const ingresarTarjeta = document.createElement("button")
const ingresarPerfil = document.querySelector("#ingresarNombre")
ingresarPerfil.addEventListener('click', perfil)
const ingresarMail = document.createElement("button")
const divIng = document.querySelector("#botonesIngresar")


ingresarTarjeta.classList.add("btn")
ingresarTarjeta.innerHTML = "Ingresar tarjeta"
ingresarTarjeta.classList.add("btn-primary")
agregarMonto.classList.add("btn-primary")

ingresarMail.classList.add("btn")
ingresarMail.innerHTML = "Ingresar mail"
ingresarMail.classList.add("btn-primary")

ingresarMail.addEventListener('click', escribirCorreo)
ingresarTarjeta.addEventListener('click', agregarTarjeta)
agregarMonto.addEventListener('click', agregarMontoATarjeta)



/* INGRESAR UN CORREO ELECTRONICO (DESDE ACA SE GENERA EL BOTON PARA AGREGAR UNA TARJETA) */
...
function escribirCorreo(){
  ...
  const { value: email } = await Swal.fire({
    ...
    input: 'email',
    inputLabel: 'mail',
    inputPlaceholder: 'Escribe tu mail'
  })
  
  if (email) {
    Swal.fire({
      icon: 'success',
      title: `Tu mail fue ingresado con exito: ${email}`,
      toast: true,
      position: 'bottom-end',
      timer: 5000,
      timerProgressBar: true
    })
  divIng.append(ingresarTarjeta)
  ingresarMail.disabled = true
...
/* AGREGAR UNA TARJETA DE CREDITO (AL INGRESAR UNA TARJETA APARECE UN BOTON PARA PODER IR AGREGANDO MONTO A LA MISMA, SOLO SE PUEDE AGREGAR 50$ USD CON CADA CLICK) */

function agregarTarjeta(){

  (async () => {
    const { value: tarjeta } = await Swal.fire({
      title: 'Ingresa una tarjeta',
      input: 'number',
      inputLabel: 'tarjeta',
      inputPlaceholder: 'Ingresar tarjeta'
    })
    const montoTarjeta = 0
    if (tarjeta) {
      Swal.fire({
        icon: 'success',
        title: `Tu tarjeta fue ingresado con exito, tu monto es de: $${montoTarjeta} USD`,
        toast: true,
        position: 'bottom-end',
        timer: 5000,
        timerProgressBar: true
      })
      divIng.append(agregarMonto)
    ingresarTarjeta.disabled = true
    }
  })()
}
...
tr.querySelector(".input___elemento").addEventListener('change', sumarCantidad)
})
totalCarrito()
...
function totalCarrito(){
  ...
  carrito.forEach((item) => {
    const totalPrecio = parseFloat(item.precio.replace('$', ''))
...
return "Por favor, escribe tu nombre"
    ...
    if (nombre) {
      ...
      onchange
divIng.append(ingresarMail)
ingresarPerfil.disabled = true
